...
1. **`src/` (Python)** — one-shot scripts triggered by GitHub Actions cron, each running for a few seconds and exiting. No persistent server. Every script except `season_reminder.py` and `season_report.py` calls `is_in_season()` right after computing `today` and returns early (no Discord call, no DB writes) outside the configured `season_config` window — see "Season boundary" below. `season_report.py` is the deliberate exception: it only makes sense *after* the season ends, so it has its own gating logic instead (see below).
   - `src/common.py` — shared helpers used by every script above: `env`/`load_dotenv`, `text` (Components V2 text block), `discord_dm`, `is_in_season`, `czech_day`/`CZECH_DAYS`, and the `DISCORD_API`/`IS_COMPONENTS_V2` constants. Each script imports what it needs rather than redefining it — mirrors the shared-helper pattern `src/retry.py` already established for retry logic. `src/season_report.py` is a partial exception: it imports `load_dotenv`/`env` from here directly, but reuses `discord_dm`/`text`/`czech_day`/`is_in_season` via its existing `completeness_sweep.X` cross-import instead (those names are just re-exports of `common`'s inside `completeness_sweep.py`'s namespace, so both call styles resolve to the same functions). `src/report_html.py` imports `czech_day` from here too. If you add a new script, pull from `common` rather than pasting another copy.
   ...
   - `src/season_report.py` — implemented (full spec in `docs/PLAN.md` Component 7 and Phase 5 checklist). Runs as a *second step* within `completeness-sweep.yml`, right after `completeness_sweep.py`, on the same Tuesday cron — no separate workflow. Does **not** call `is_in_season()`; instead reads `season_config` directly and exits unless `today > season_end` (a `--as-of YYYY-MM-DD` CLI flag overrides "today" for testing before the real season ends). Once past `season_end`: reuses `completeness_sweep.py`'s `fill_missing_actual_weather`/`fetch_missing_days`/nag-building functions (cross-module import) — bounded by `season_end`, not real "today" — to keep covering the post-season completeness check that `completeness_sweep.py` itself stops doing once `is_in_season()` goes false. If anything's still missing, sends the nag and stops there. If everything's logged, builds `reports/season-<year>.html` via four helper modules (`report_stats.py` → `report_charts.py` → `report_narrative.py` → `report_html.py`), DMs it to Discord as a file attachment (`discord_dm_with_file`), and stamps `season_config.report_sent_at` so it doesn't regenerate on the next run.
     ...
     - `src/report_narrative.py` — calls Google Gemini (`gemini-3.5-flash`, pinned deliberately as `DEFAULT_MODEL`, not the newest flash model available — overridable via `GEMINI_MODEL`) over raw HTTPS via `retry.request_with_retry`, given only the precomputed stats JSON; never invents numbers.
...
- **Components V2 + file attachments**: a bare multipart file upload alongside a Components V2 payload is silently dropped by Discord (200 OK, no attachment) — confirmed by a real smoke test. It needs an explicit `attachments: [{"id": 0, "filename": ...}]` entry in the JSON payload mapping the upload to its `files[0]` multipart part, *and* a `File` component (`{"type": 13, "file": {"url": "attachment://<filename>"}}`) in `components` so it actually renders. `src/season_report.py`'s `discord_dm_with_file` is the only place in this codebase that sends an attachment, and does both — copy that pattern, not `discord_dm`, for any future file-sending code.
...
- All day names are rendered in Czech (`CZECH_DAYS`, defined once in `src/common.py` on the Python side), computed by hand rather than via locale-dependent `%A` — GitHub's runners don't reliably have a Czech locale.
...
- The Worker's `CZECH_DAYS` array (`worker/src/index.ts`) starts at Sunday (JS `getUTCDay()` convention: 0=Sunday); Python's (`src/common.py`) starts at Monday (`date.weekday()` convention). Don't copy one array into the other without adjusting the offset — these two are the only remaining copies, by design (TS and Python don't share code).
