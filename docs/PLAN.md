...
- [ ] Error handling / hardening — broken down:
  - [ ] Offset all four workflow cron schedules a few minutes off `:00` (`forecast.yml`, `monday-log.yml`, `completeness-sweep.yml`, `season-reminder.yml`) — GitHub's own docs name the top of every hour as the highest-delay-risk slot for scheduled runs; this is their documented mitigation, not a full fix (GitHub gives no upper bound on delay and admits a run can be skipped outright under high load).
  - [ ] Guard `forecast.py`'s Thu/Fri branch (`today.weekday() == 4` at the bottom of the script, decides whether the last-weekend recap gets prepended) against a severely delayed run landing on the wrong calendar day — e.g. a delayed Friday run rolling past midnight into Saturday would silently take the Thursday-style branch (no recap, `predikce_den` mislabeled) instead of failing loudly. Log/flag when the actual weekday doesn't match what the workflow name assumes, rather than silently mis-branching.
  - [ ] Shared retry-with-backoff helper for transient HTTP failures (timeout/429/5xx only, not 4xx) — every `requests.get`/`requests.post` in `src/` currently has a `timeout=` but no retry: Open-Meteo forecast + archive calls in `forecast.py`, and the Discord DM-open + send/edit calls duplicated across `forecast.py`, `log_message.py`, `completeness_sweep.py`, `season_reminder.py`. One transient Discord 503 today = a fully silent missed forecast.
  - [ ] Light retry on `psycopg.connect(..., connect_timeout=10)` calls (four call sites) for transient Neon cold-start/connection blips.
  - [ ] Season-modal date sanity check (Worker `parseDayMonth`/`season_modal` handler) — the season formula itself ("first weekend of May → first weekend of October") never crosses a DST boundary by construction, so this isn't a DST fix; it's cheap insurance against a fat-fingered `season_end` (e.g. `31.10.` instead of `4.10.`) slipping through the free-text modal with no range check.
